    ...
    <xsl:copy>
      ...
      <xsl:for-each select="//t:div[@type='edition']">
        <TEI>
          ...
          <text>
            <body>
              <xsl:apply-templates select="doc(concat($cwd,'/epidoc/',count(preceding::t:div[@type='edition']), '.xml'))" mode="import"><xsl:with-param name="id" select="concat('ed',count(preceding::t:div[@type='edition']) + 1)" tunnel="yes"></xsl:with-param></xsl:apply-templates>
            </body>
          </text>
        </TEI>
      </xsl:for-each>
